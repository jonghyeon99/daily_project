...
FROM (SELECT CATEGORY, MAX(PRICE) AS MAX_PRICE
      ...
      GROUP BY CATEGORY) a
JOIN FOOD_PRODUCT b
ON a.CATEGORY = b.CATEGORY and MAX_PRICE = b.price
ORDER BY MAX_PRICE DESC;

-- 이게 답이지만 내가 처음 적은 것은 아래다.
...
-- 이것도 결과가 똑같이 나왔는데 계속 틀렸다고 그래서 결국 gpt한테 물어보니
-- MAX(PRICE)는 제대로 나오지만 PRODUCT_NAME이 맞게 안 나온다고 한다.
-- 결국 나 혼자 풀 수 없었다.
-- 서브쿼리랑 조인을 좀 더 공부해야겠다.